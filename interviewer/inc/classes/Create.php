...
*/
    ...
    public function createScore($data){
      if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submitScore'])) {
        ...
        //form server validation
        //check input field empty or not
        if (empty($btech) || empty($mtech) || empty($exp) || empty($com)){
          $msg = '<div class="alert alert-danger"><b>Form Fillup Error!</b> Fields Must Not be Empty</div>';
          return $msg;
          exit();
        }

        $total = $btech + $mtech + $exp + $com;

        //insert data to databsae if everything is okay
        $sql = "insert into scoresheet(btech, mtech, exp, communication, total, cand_id, inter_id) values(?,?,?,?,?,?,?)";
        $arr = array($btech, $mtech, $exp, $com, $total, $cand_id, $inter_id);
        $results = $this->db->simplequery($sql, $arr);
        if ($results) {
          $msg = '<div class="alert alert-success"><b>Score added success!</b> You have successfully added a new score.</div>';
          return $msg;
        } else {
          $msg = '<div class="alert alert-danger"><b>Score add Error!</b> Sorry, Some Unexpected Error Occur, Please try again</div>';
          return $msg;
        }
